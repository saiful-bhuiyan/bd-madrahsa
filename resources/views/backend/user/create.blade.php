                    ...
                    <form method="post" action="{{route('user.store')}}" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-4 col-md-6 col-12" id="inputSingleBox">
                                <div>
                                <label for="defaultFormControlInput" class="form-label">@lang('user.role') <span class="text-danger">*</span></label>
                                <select class="form-control" name="role_id" required>
                                    <option>@lang('common.select_one') </option>
                                    @if($user_role)
                                    @foreach($user_role as $view_role)
                                    <option value="{{$view_role->id}}">@if($lang == 'en'){{$view_role->role_name_en}}
                                        @elseif($lang == 'bn'){{$view_role->role_name_bn}}@endif</option>
                                    @endforeach
                                    @endif
                                </select>
                                </div>
                            ...
                            <div class="col-lg-4 col-md-6 col-12" id="inputSingleBox">
                                <div>
                                <label for="defaultFormControlInput" class="form-label">@lang('user.image') <span class="text-danger">*</span></label>
                                <input
                                    type="file"
                                    class="form-control"
                                    id="defaultFormControlInput"
