...
<head>
    ...
    <title>@lang('common.admin')    @lang('common.panel')</title>
        ...
        <header class="header">
            <div class="page-brand">
                <a class="link" href="index.html">
                    <span class="brand">@lang('common.admin')   @lang('common.panel')</span>
                    </span>
                    <span class="brand-mini">@lang('common.short')</span>
                </a>
            </div>
                    ...
                    <li>
                        <form class="navbar-search" action="javascript:;">
                            <div class="rel">
                                ...
                                <input class="form-control" placeholder="@lang('common.search')">
                            </div>
                        </form>
                    </li>
                </ul>
                ...
                <ul class="nav navbar-toolbar">
                    ...
                    <li class="dropdown dropdown-user">
                        <a class="nav-link dropdown-toggle link" data-toggle="dropdown">
                            <img src="{{asset('backend/img/UserImage')}}/{{Auth::user()->image}}" />
                            <span></span>@if($lang == 'en'){{$role->role_name_en}}@elseif($lang == 'bn'){{$role->role_name_bn}}@endif<i class="fa fa-angle-down m-l-5"></i></a>
                        <ul class="dropdown-menu dropdown-menu-right">
                            <a class="dropdown-item" href="profile.html"><i class="fa fa-user"></i>@lang('common.profile')</a>
                            <a class="dropdown-item" href="profile.html"><i class="fa fa-cog"></i>@lang('common.setting')</a>
                            <a class="dropdown-item" href="javascript:;"><i class="fa fa-support"></i>@lang('common.support')</a>
                            <li class="dropdown-divider"></li>
                            <a class="dropdown-item" href="{{url('adminLogout')}}"><i class="fa fa-power-off"></i>@lang('common.logout')</a>
                        </ul>
                    </li>
                </ul>
        ...
        <nav class="page-sidebar" id="sidebar">
            <div id="sidebar-collapse">
                <div class="admin-block d-flex">
                    <div>
                        <img src="{{asset('backend/img/UserImage')}}/{{Auth::user()->image}}" width="45px" />
                    </div>
                    <div class="admin-info">
                        <div class="font-strong">{{Auth::user()->name}}</div><small>@if($lang == 'en'){{$role->role_name_en}}@elseif($lang == 'bn'){{$role->role_name_bn}}@endif</small></div>
                </div>
                <ul class="side-menu metismenu">
                    ...
                    @php 
                    $check_parent = DB::table('parent_has_permissions')->where('role_id',Auth::user()->role_id)->where('parent_id',$m->id)->count();
                    @endphp
                    ...
                    <li>
                        ...
                        <a href="javascript:;"><i class="sidebar-item-icon {{$m->icon}}"></i>
                            <span class="nav-label">@if($lang == 'en'){{$m->menu_name_en}}@elseif($lang=="bn"){{$m->menu_name_bn}}@endif</span><i class="fa fa-angle-left arrow"></i></a>
                        <ul class="nav-2-level collapse">
                            ...
                            $check_parent_module = DB::table('module_has_permissions')->where('role_id',Auth::user()->role_id)->where('module_id',$sm->id)->count();

                            @endphp
                            
                            @if($check_parent_module > 0)
                            <li>
                                <a href="@if($sm->route_name != null){{url($sm->route_name)}}@endif">@if($lang == 'en'){{$sm->menu_name_en}}@elseif($lang=="bn"){{$sm->menu_name_bn}}@endif</a>
                            </li>
                    ...
                    $module_check = DB::table('module_has_permissions')->where('role_id',Auth::user()->role_id)->where('module_id',$m->id)->count();
                    @endphp

                    @if($module_check > 0)
                    <li>
                        <a class="active" href="{{url($m->route_name)}}"><i class="sidebar-item-icon {{$m->icon}}"></i>
                            <span class="nav-label">@if($lang == 'en'){{$m->menu_name_en}}@elseif($lang=="bn"){{$m->menu_name_bn}}@endif</span>
                        </a>
                    </li>
                    @endif
